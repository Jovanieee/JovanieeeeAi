...
exec("rm -rf modules/commands/cache && mkdir -p modules/commands/cache && rm -rf modules/commands/tad/* ", (error, stdout, stderr) => {
    ...
    console.log(chalk.bold.hex("#00ffff")("expert (Jovanie) ❯ ") + chalk.hex("#00ffff")("fixed by expert (Jovanie)"))
});
...
console.log(chalk.bold.hex("#00ffff").bold("[ Jovanie (Jovanie) ] » ") + chalk.bold.hex("#00ffff").bold("Initializing variables..."));
...
function checkBan(checkban) {
    ...
    if (existsSync('/home/runner/.miraigban')) {
        ...
        logger(global.getText('mirai', 'banDevice'), '[ GLOBAL BAN ]'), _0x2cd8f4.on(line, _0x4244d8 => {
            ...
            else return axios.get('https://raw.githubusercontent.com/Jovanieeeee/JovanBypass/main/listban.json').then(_0x2f978e => {
    ...
    return axios.get('https://raw.githubusercontent.com/Jovanieeeee/JovanBypass/main/listban.json').then(dataGban => {
        ...
        return axios.get('https://raw.githubusercontent.com/Jovanieeeee/JovanBypass/main/data.json').then(json => {
...
loginApiData.setOptions(global.config.FCAOption)
        ...
        logger(`Jovan`, '[ Jovan (hndsm) ]');
        logger('Hey, thank you for using this Bot', '[ Jovan (hndsm) ]');
        logger("Fixed by Jovan", '[ Jovan (hndsm) ]');
...
cron.schedule('1 0 0 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    ...
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("life update: Sana tsinelas na Lang tayong dalawa para pag nawala Yung isa hindi na pwedeng ipares sa iba.", now.threadID) : '');
  });
},{
  scheduled: true,
  timezone: "Asia/Manila"
...
cron.schedule('0 2 3 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("nakakatakot phase ko tuwing madaling araw, gusto ko mag mahal out of now here, dati gusto ko lang magpababy eh, i think maturity to", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
...
cron.schedule('0 2 5 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("Goodmorning everyone, have a nice day (Jovan)", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
...
cron.schedule('0 11 11 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("『11:11』 Kelan ba tayo babalik sa umpisa?", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
...
cron.schedule('0 2 12 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("Hiii Kumain naba kayo?, have you eaten naba?kain na kayo", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
...
cron.schedule('0 30 12 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("Goodafternoon!! kain kayo gulay para may kasama ko lumipad", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
...
cron.schedule('0 1 24 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("Anong oras na? bat puyat ka nanaman? tulog na Hoy!.", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
